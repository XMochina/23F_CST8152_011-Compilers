	...
	while (1) { /* endless loop broken by token returns it will generate a warning */
		...
		*/
		...
		case '}':
			...
			scData.scanHistogram[currentToken.code]++;
			return currentToken;
		/* Cases for arithmetic symbols */
		case '+':
			newc = readerGetChar(sourceBuffer);
			if (newc == '+') { // ++
				currentToken.code = UNOP_T;
				scData.scanHistogram[currentToken.code]++;
				currentToken.attribute.unaryOperator = OP_UAD;
				return currentToken;
			} else if (newc == '=') { // +=
				currentToken.code = ASOP_T;
				scData.scanHistogram[currentToken.code]++;
				currentToken.attribute.assignmentOperator = OP_AAD;
				return currentToken;
			} else { // +
		...
		case '-':
			...
			if (newc == '-') { // --
				currentToken.code = UNOP_T;
				scData.scanHistogram[currentToken.code]++;
				currentToken.attribute.unaryOperator = OP_USU;
				return currentToken;
			} else if (newc == '=') { // -=
				currentToken.code = ASOP_T;
				scData.scanHistogram[currentToken.code]++;
				currentToken.attribute.assignmentOperator = OP_ASU;
				return currentToken;
			} else { // -
				readerRetract(sourceBuffer);
				currentToken.code = AROP_T;
		...
		case '*':
			...
			if (newc == '=') { // *=
				currentToken.code = ASOP_T;
				scData.scanHistogram[currentToken.code]++;
				currentToken.attribute.assignmentOperator = OP_AMU;
				return currentToken;
			} else {
				currentToken.code = AROP_T; // *
				scData.scanHistogram[currentToken.code]++;
		...
		case '%':
			...
			return currentToken;
		/* Cases for forward slash symbols */
		case '/':
			newc = readerGetChar(sourceBuffer);
			if (newc == '/') { // single line comment
				do {
					c = readerGetChar(sourceBuffer);
				...
				currentToken.code = ASOP_T;
				scData.scanHistogram[currentToken.code]++;
				currentToken.attribute.assignmentOperator = OP_ADI;
				return currentToken;
			} else if (newc == '*') { // /*
				check = False;
				do {
					c = readerGetChar(sourceBuffer);
					if (check && c != '/') { // not the end of the comment
						check = False;
					}
					if (c == CHARSEOF0 || c == CHARSEOF255) {
						readerRetract(sourceBuffer);
				...
				} while (!(check && c == '/') && c != CHARSEOF0 && c != CHARSEOF255);
				break;
			} else { // /
				readerRetract(sourceBuffer);
				currentToken.code = AROP_T;
		...
		case '=':
			...
			if (newc == '=') { // ==
				...
				currentToken.code = ASOP_T;
				scData.scanHistogram[currentToken.code]++;
				currentToken.attribute.assignmentOperator = OP_AAS;
				return currentToken;
			}
		case '&':
			newc = readerGetChar(sourceBuffer);
		...
		case ':':
			...
			if (newc == '=') { // :=
				currentToken.code = ASOP_T;
				scData.scanHistogram[currentToken.code]++;
				currentToken.attribute.logicalOperator = OP_ASH;
				return currentToken;
			}
 ...
 ***********************************************************
 */
/* TO_DO: Adjust the function for single line comment */
...
/* TO_DO: Adjust the function for multi line comment */
...
func printToken(Token t) {
	...
	case FNI_T:
		...
		break;
	case STL_T:
		printf("STL_T\t\t%d\t ", (int32)t.attribute.codeType);
