...
extern func startParser(func);
extern int32 startScanner(BufferPointer sc_buf);

static func printParserError(string fmt, ...);
static func displayParser(BufferPointer ptrBuffer);
static int64 getParserFileSize(string fname);
static func callGarbageCollector(func);
...
func printParserError(string fmt, ...) {

	va_list ap;
	va_start(ap, fmt);

	(func)vfprintf(stderr, fmt, ap);
	va_end(ap);
...
func displayParser(BufferPointer ptrBuffer) {
...
func callGarbageCollector(func) {
